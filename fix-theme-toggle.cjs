...
if (!cssContent.includes('.theme-switch {')) {
  fs.writeFileSync('src/index.css', cssContent + '\n' + newCss);
}


// Now modify ImageEditor.tsx
let tsxContent = fs.readFileSync('src/components/ImageEditor.tsx', 'utf8');
...
const replacementHtml = `
          <div className="relative">
            ...
            <label className="theme-switch mt-1">
              <input 
...
const oldButtonRegex = /<button[\s\S]*?title="Toggle Theme"[\s\S]*?<\/button>/;
tsxContent = tsxContent.replace(oldButtonRegex, replacementHtml);

fs.writeFileSync('src/components/ImageEditor.tsx', tsxContent);
